...
/**
 * Looks up the display label for an investigation id.
 *
 * @param {Array<{ _id: string, label: string }>} [investigations=[]]
 * @param {string} investigation - The investigation `_id` to match.
...
export function getInvestigationLabel(investigations = [], investigation) {
	const id = investigation != null ? String(investigation) : '';
	const match = investigations.find((item) => String(item._id) === id);
	return match?.label ?? investigation;
}

/**
 * Looks up the unit of measure for an investigation id.
 *
 * @param {Array<{ _id: string, unit?: string }>} [investigations=[]]
 * @param {string} investigation - The investigation `_id` to match.
