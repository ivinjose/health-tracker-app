...
export function isExistingReportFile(file) {
	return Boolean(file && file.existing);
}

export function existingReportFile(filename) {
	if (!filename) return undefined;
	return { existing: true, name: filename };
}

export function getReportFileLabel(file) {
	if (isMissingReportFile(file)) return '';
	if (isExistingReportFile(file)) return 'Attached report';
...
export function isPdfOrImage(file) {
	if (isMissingReportFile(file) || isExistingReportFile(file)) return true;
...
export function isLocalReportFile(file) {
	if (isMissingReportFile(file) || isExistingReportFile(file)) return false;
	return Boolean(file.uri || isBrowserFile(file) || isBrowserFile(file.file));
}

export async function readReportFileBytes(file) {
	...
	if (file?.uri) {
		...
		if (!response.ok) {
			throw new Error('Could not read this report.');
		}
...
function normalizeLabelIds(labels) {
...
export function buildCreateReportRequest(data) {
	const { investigation, value, date, remarks, appointment, report, labels } = data;
	...
	if (isMissingReportFile(report) || isExistingReportFile(report)) {
	...
	appendReportFile(formData, report);

	return { body: formData, config: multipartRequestConfig() };
}

function appendReportFile(formData, report) {
...
function toBulkReportFields(row) {
	const parsedNumber = Number(row?.value);

	if (!row?.investigation || !parsedNumber || !row?.date) {
		throw new Error('Could not create reports.');
	}
	...
	return {
		...
		remarks: row.remarks,
		labels: normalizeLabelIds(row.labels),
	};
}

export function buildCreateReportsRequest(rows, report) {
	if (!Array.isArray(rows) || rows.length === 0) {
		throw new Error('Could not create reports.');
	}

	const reports = rows.map((row) => toBulkReportFields(row));

	if (isMissingReportFile(report) || isExistingReportFile(report)) {
		return { body: { reports }, config: undefined };
	}
...
export function buildUpdateReportRequest(data) {
	const { id, investigation, value, date, remarks, report, labels } = data;
	const parsedNumber = Number(value);
	...
	if (isExistingReportFile(report)) {
		return { body: fields, config: undefined };
	}

	if (isMissingReportFile(report)) {
		return { body: { ...fields, removeFile: true }, config: undefined };
	}
