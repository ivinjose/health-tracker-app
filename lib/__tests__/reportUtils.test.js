...
const investigations = [
	{ _id: 'glucose-id', label: 'Glucose', unit: 'mg/dL' },
	{ _id: 'tsh-id', label: 'TSH', unit: 'mIU/L' },
	{ _id: 'notes-id', label: 'Notes' },
];

describe('getInvestigationLabel', () => {
	it('returns the matching label when the id exists', () => {
		expect(getInvestigationLabel(investigations, 'glucose-id')).toBe('Glucose');
	});

	it('stringifies ObjectId-like values before matching', () => {
		expect(getInvestigationLabel(investigations, { toString: () => 'glucose-id' })).toBe(
			'Glucose'
		);
	});
	...
	it('uses an empty investigations list by default', () => {
		expect(getInvestigationLabel(undefined, 'glucose-id')).toBe('glucose-id');
	});
});

describe('getInvestigationUnit', () => {
	it('returns the matching unit when present', () => {
		expect(getInvestigationUnit(investigations, 'glucose-id')).toBe('mg/dL');
	});

	it('returns an empty string when the investigation has no unit', () => {
		expect(getInvestigationUnit(investigations, 'notes-id')).toBe('');
	});

	it('returns an empty string when no match is found', () => {
		expect(getInvestigationUnit(investigations, 'missing')).toBe('');
		expect(getInvestigationUnit(undefined, 'glucose-id')).toBe('');
	});
});
