...
describe('createReportPreview', () => {
	...
	it('uses a mime hint when the filename is generic', () => {
		const preview = createReportPreview(new Uint8Array([1, 2, 3]), 'report', 'image/jpeg');
		expect(preview.kind).toBe('image');
		expect(preview.mime).toBe('image/jpeg');
		expect(preview.dataUri).toBe('data:image/jpeg;base64,AQID');
	});
});
