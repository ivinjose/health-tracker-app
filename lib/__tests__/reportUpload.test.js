...
describe('picked file helpers', () => {
	...
	it('treats a picked uri as a local file, not an existing server attachment', () => {
		expect(isLocalReportFile({ uri: 'file:///tmp/a.pdf', name: 'a.pdf' })).toBe(true);
		expect(isLocalReportFile({ existing: true, name: 'profile-1_1_abc.png' })).toBe(false);
		expect(isLocalReportFile(undefined)).toBe(false);
	});
});

describe('readReportFileBytes', () => {
	...
	beforeAll(() => {
		...
		global.File = class File {
			constructor(parts, name, options = {}) {
				...
				this._bytes = parts[0] instanceof Uint8Array ? parts[0] : new Uint8Array(parts[0]);
			}

			arrayBuffer() {
				const { _bytes: bytes } = this;
	...
	it('reads bytes from a browser File', async () => {
		const file = new File([new Uint8Array([1, 2, 3])], 'a.png', { type: 'image/png' });
		const bytes = await readReportFileBytes(file);
		expect(new Uint8Array(bytes)).toEqual(new Uint8Array([1, 2, 3]));
	});

	it('reads bytes from a nested browser File', async () => {
		const inner = new File([new Uint8Array([4, 5])], 'a.pdf', { type: 'application/pdf' });
		const bytes = await readReportFileBytes({
			uri: 'blob:abc',
			name: 'a.pdf',
			file: inner,
		});
	...
	it('fetches bytes from a uri when no File is present', async () => {
		...
		expect(global.fetch).toHaveBeenCalledWith('file:///tmp/a.pdf');
	});

	it('throws when fetch fails', async () => {
		global.fetch = jest.fn().mockResolvedValue({ ok: false });
		await expect(readReportFileBytes({ uri: 'file:///tmp/a.pdf' })).rejects.toThrow(
			'Could not read this report.'
		);
	});

	it('throws when there is nothing to read', async () => {
		await expect(readReportFileBytes({})).rejects.toThrow('Could not read this report.');
	});
});
...
describe('buildCreateReportRequest', () => {
	...
	it('sends catalog label ids on JSON create', () => {
	...
	it('sends multipart when a file is attached', () => {
		...
		try {
			...
			expect(append).toHaveBeenCalledWith('labels', '[]');
...
describe('buildCreateReportsRequest', () => {
	const date = new Date(1700000000000);
	const row = {
		investigation: '6a8962dd0274ed29b52dd702',
		value: '6.5',
	...
	const second = {
		investigation: '7a8962dd0274ed29b52dd703',
		value: '7.1',
		date,
		remarks: '',
	};

	it('throws when the row list is empty or a row is incomplete', () => {
		expect(() => buildCreateReportsRequest([])).toThrow('Could not create reports.');
		expect(() => buildCreateReportsRequest([{ ...row, investigation: '' }])).toThrow(
			'Could not create reports.'
		);
	});

	it('sends JSON with every row and no file', () => {
		const { body, config } = buildCreateReportsRequest([row, second]);
		expect(config).toBeUndefined();
		expect(body).toEqual({
			reports: [
				{
	...
	it('sends one multipart file for the whole list', () => {
		const append = jest.spyOn(FormData.prototype, 'append');
		const file = {
			uri: 'file:///tmp/a.pdf',
			name: 'a.pdf',
		...
		try {
			const { body, config } = buildCreateReportsRequest([row, second], file);
			expect(body).toBeInstanceOf(FormData);
			expect(config.headers['Content-Type']).toBe('multipart/form-data');
			expect(append).toHaveBeenCalledWith(
				'reports',
			...
			expect(append.mock.calls.filter((call) => call[0] === 'report')).toHaveLength(1);
		} finally {
			append.mockRestore();
		}
	});
...
describe('buildUpdateReportRequest', () => {
	...
	it('sends catalog label ids on JSON update', () => {
		const { body } = buildUpdateReportRequest({
			...base,
			report: existingReportFile('profile-1_1_abc.png'),
			labels: ['aaaaaaaaaaaaaaaaaaaaaaaa'],
		});
		expect(body.labels).toEqual(['aaaaaaaaaaaaaaaaaaaaaaaa']);
	});
...
describe('bulk row labels', () => {
	...
	it('copies label ids onto every bulk row', () => {
		const { body } = buildCreateReportsRequest([
			{ ...row, labels: ['aaaaaaaaaaaaaaaaaaaaaaaa'] },
			{ ...row, value: '7.1', labels: ['aaaaaaaaaaaaaaaaaaaaaaaa'] },
		]);
		expect(body.reports[0].labels).toEqual(['aaaaaaaaaaaaaaaaaaaaaaaa']);
		expect(body.reports[1].labels).toEqual(['aaaaaaaaaaaaaaaaaaaaaaaa']);
	});
});
