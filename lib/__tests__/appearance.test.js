...
describe('appearance constants', () => {
	it('exposes the known palette names and storage key', () => {
		expect(APPEARANCE_NAMES).toEqual(['light', 'dark']);
		expect(APPEARANCE_STORAGE_KEY).toBe('appearance');
	});
});
...
describe('getAppearance', () => {
	...
	it('builds light appearance tokens from the light palette', () => {
		...
		expect(appearance.layout).toEqual(getAppearance('dark').layout);
